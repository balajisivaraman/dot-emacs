    ...
    (condition-case-unless-debug err
        (if-let* ((buffer (pop-to-buffer-same-window "*elpaca-bootstrap*"))
                  ((zerop (apply #'call-process "git" nil buffer t "clone"
                                 (append (list (format "--depth=%d" (or (plist-get order :depth) 1))
                                               "--no-single-branch")
...
;; Wait for elpaca-use-package-mode to activate before any use-package :ensure t calls.
(elpaca-wait)

;;; Load path for local modules.
